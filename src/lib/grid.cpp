...
grid::grid(unsigned char depth, const ::bbox& box) : m_depth(depth), 	m_bbox(box), m_data(depth-1) {
...
const unsigned grid::depth() const {
	return m_depth;
}
...
void grid::visit_active(const std::function<void (const ::bbox&)>& visitor, unsigned max_level) {
	m_data.visit_active(visitor, m_bbox, max_level);
}
